...
## ADR-071 — First-party control of one platform WhatsApp sender

Accepted. My Kustomers Admin manages a narrow, separately authenticated gateway control API, never the upstream dashboard. Keep one canonical session, distinct delivery/control credentials, masked read projections and ephemeral no-store QR. Privileged changes require actual active Super Admin, AAL2, confirmation, reason and append-only audit. A private SQL pause preserves pending work; durable gateway pause serializes handoffs against replacement. Explicit verified resume avoids abandoned pairing silently reopening delivery. Gateway control IDs reuse a namespaced existing ledger; no customer message is generated by the control plane. General rollout and a dedicated number remain separate approval gates.

## 2026-09-24 — Reschedule through READY without undoing completed work

Supersedes earlier pre-work-only eligibility in this document. Allow DRAFT,
