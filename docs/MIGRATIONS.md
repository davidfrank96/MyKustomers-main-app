...
| `20260902104919_customer_email_source_of_truth.sql`                        | Exact approved SHA-256 `83f6bb408555fb19cd3c10c6ddc80821b1da6932fba0df18a1d2d9ec8b9059eb`; applied transactionally to the Production-backed project after rollback compile; seven postgres-owned empty-search-path functions, exact grants, no profile fallback/write, unchanged suspect-address counts, and rollback-only booking-contact/no-email-delivery behavior passed                |
...
## 2026-09-02 Customer Email Source-Of-Truth Migration

The user explicitly approved exact migration
`20260902104919_customer_email_source_of_truth.sql` after the repository and
Production provenance audit established that the reported address did not exist
in code, history, customer/booking/outbox/Auth/audit data, or provider attempts.
The SHA-256-frozen file applied as one transaction to the configured
Production-backed database after a rollback-only compile.

The migration replaces seven functions without changing signatures, tables,
columns, enums, indexes, RLS, providers, or environment values. Public
confirmation no longer writes `customers.email`; amendment, add-on, reschedule,
cancellation, and delivery recipient selection no longer fall back to it; and
no-email delivery satisfies the strict future-write invariant with its version
1 feedback capability even when no email event exists.

Post-apply catalog inspection confirmed postgres ownership, empty search paths,
expected service/authenticated grants, no public/anonymous execution, no profile
email references, and no customer-email writes. A controlled synthetic
transaction proved normalized booking contact with unchanged saved profile plus
...
Repository migration generated with the Supabase CLI. APPLIED under the subsequent explicit single-business production approval. Adds channel preferences/outbox and narrow wrappers, preserving existing domain function ACLs and email behavior. Requires a separately provisioned Vault key and explicit private pilot row before enabling opt-in. Migration itself contains neither key nor real pilot UUID. Passed disposable native PostgreSQL verification, including a local schema-only export with original lifecycle RPCs. Production-backed rollback testing was blocked by automatic approval review and was not run; the disposable local alternative passed before the separately approved real migration. See [deployment order](WHATSAPP_PILOT.md).

## WhatsApp Phase 3 entitlement migration — 2026-09-23

`20260923022729_business_feature_entitlements.sql`: additive, manually reviewed and verified in fresh native PostgreSQL clusters, including the full baseline schema/lifecycle. It adds the feature table/read policy, audited AAL2 mutation, rollout-read RPC and entitlement checks to four existing functions while preserving their ACLs. No production application from CI; controlled production application and the sole PILOT grant remain release gates. Phase 2 migration `20260923001137_whatsapp_pilot_channel.sql` was subsequently applied under explicit approval and released with PR #89. See [Phase 3 release state](WHATSAPP_PHASE3.md).
