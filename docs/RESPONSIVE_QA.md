...
## 2026-09-18 money input / notification follow-up

The shared amount input now carries an explicit currency prefix and secondary
compact helper; exact records retain full amounts. Notification rows close the
shared dialog on ordinary same-window activation before the resolver performs
its existing document navigation and business switch. Modified clicks retain
the original dialog. No bottom offsets, viewport JavaScript, safe-area changes
or service-worker rewrite were introduced.

Cross-engine matrix, screenshots and physical-device limitations are recorded in
[BOOKING_CURRENCY_MONEY_PWA.md](BOOKING_CURRENCY_MONEY_PWA.md). Physical iPhone
installed-PWA push/nav recovery remains unverified; modal cleanup evidence must
not be presented as proof of the reported OS-notification displacement.
