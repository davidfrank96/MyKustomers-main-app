...
`npx playwright test --config playwright.whatsapp.config.ts` uses loopback synthetic fixtures and blank gateway credentials. Six Chromium/WebKit tests cover all ten requested widths, keyboard/consent reset, uncertain status, stop preserving Email and hidden non-pilot choices. Screenshots are local ignored artifacts. The final browser run passed all six tests; protected Runtime Security remains SKIPPED without an approved non-production target. Production controlled sends were subsequently verified under explicit approval for PR #89 release; global sending was restored OFF. See [remaining release gates](WHATSAPP_PILOT.md).
...
Additional regression evidence: native Notification Contracts SQL passed and its local browser matrix passed 24/24. CI on implementation commit `d6a3213` passed 1129 unit/integration/static tests (24 existing opt-in skips), Quality, Build, Dependency Security and all 49 Profile/Social checks. Local Profile/Social initially had two navigation failures (pending RSC prefetch and WebKit cancelled requests); an unchanged-main comparison passed, the WebKit focused rerun passed, and a temporary Chromium diagnostic copy with identical assertions also passed. No product or assertion change was made for these intermittent local results. The first CI Notification Contracts browser run failed one WebKit golden-shell case on a Next dev-tools stack-frame fetch; two focused local repeats passed unchanged. CI must still be green before merge; these reruns do not erase the failed run.

Application [draft PR #89](https://github.com/davidfrank96/MyKustomers-main-app/pull/89) and separate [gateway draft PR #1](https://github.com/davidfrank96/wa-akg/pull/1) contain the reviewable changes. Production migration/Vault/pilot settings and live sends remain unapplied.

The CI WebKit golden-shell failure repeated on the documentation-only follow-up. The test now waits for network quiescence before screenshots/forced navigations and before its final error assertion, allowing hydration and development stack-frame requests to settle. The error assertion is unchanged; no errors are filtered or suppressed. Focused Chromium and WebKit verification passed both cases after this synchronization change. The earlier credential-backed CI E2E run completed with 83 passed / 19 existing opt-in skips.

## WhatsApp Phase 3 verification

Focused entitlement server/Admin cases cover fail-closed reads, kill switch, operational denial, AAL2 authorization, reason validation, authenticated mutation and database failure. Disposable native SQL tests cover RLS, cross-tenant access, grant/revoke audit, forged creation, enqueue/claim/dispatch denial, Email preservation and no replay. The full original lifecycle replay against a schema-only snapshot passes all nine events across three channel choices. Browser fixtures add existing/local/empty/new customer phone handling, customer/recipient consent resets, Settings/Admin grant/revoke, history preservation and the exact ten-size matrix. Local synthetic fixtures never send externally. Physical keyboard behavior is distinct from emulated viewport contraction; protected cloud Runtime Security remains SKIPPED without an approved non-production target. Final suite/release outcomes are recorded in [Phase 3](WHATSAPP_PHASE3.md).


## WhatsApp Admin control plane — 2026-09-23

WhatsApp Admin coverage includes separate-key gateway denial, masked response reconstruction, QR no-store/expiry/connected denial, durable pause/idempotent controls, cleanup ordering and failed unlink preservation. App tests cover role/MFA/audit-before-handoff, server-only attestation, no retries of uncertain control, and sanitized failures. Chromium/WebKit fixtures cover ten viewports, connected/danger UI, pairing polling/close/connection cleanup and vendor denial. QR is never captured in screenshots/traces. Production acceptance is strictly read-only.

Local release evidence: 1,163 unit/integration tests passed with 24 existing opt-in skips; E2E 40 passed / 62 gated skips; Notification UI 24 passed; WhatsApp UI 18 passed, including eight new Admin cases; native Notification and WhatsApp SQL passed. Runtime Security is SKIPPED (all 21 guarded tests). Typecheck passes after correcting a new test's literal action type, and navigation expectations include the eighth Admin destination. Initial Profile/Social run: 47 passed / two navigation failures; the destination case passed unchanged on rerun. The logout fixture now waits for login-page RSC prefetches to settle before forced navigation, retaining every error/security assertion. Final CI and read-only Production acceptance remain release gates.

The unchanged Profile destination file rerun passed all eight cases across both engines; the synchronized logout file passed all four. The optimized production build used by both focused runs passed. App lint, typecheck, zero-vulnerability audit and diff check pass. This records the initial failures rather than treating the full first run as green; the final full Profile/Social CI job remains required.


## Customer updates UI polish — 2026-09-24

UI-only Customer updates pass: existing five-file component/booking regression selection passed 22 tests. Existing WhatsApp channel browser suite passed all 10 Chromium/WebKit journeys, including ten viewport sizes, default Email, independent channel selection, conditional WhatsApp number, phone prefill, customer/recipient consent resets, hidden non-entitled choices and unchanged booking-detail status/stop behavior. Focused ESLint, typecheck and diff check passed. Fixtures are isolated and synthetic; no production messages, mutations or rollout verification were performed.


## Customer updates summary — 2026-09-24

New focused summary coverage verifies authoritative channel combinations, legitimate stopped-neither state, paused delivery, retained uncertain history, legacy defaults, unavailable reads, hidden empty data and history retained after entitlement removal. Local browser fixtures add explicit summary preference/event scenarios without changing the previous default fixtures. The existing editor and stop-flow regression suite remains unchanged. No live data or messages are used. Final local verification: lint passed (zero errors; one pre-existing unused-import warning in control-fixture.mjs), typecheck passed, full unit/integration suite 1,172 passed / zero failed / 24 existing skips (182 passing files / 21 skipped files), optimized production build passed, and diff check passed. Chromium/WebKit channel regressions plus the new summary suite passed all 20 tests with zero failures/skips. Nine new summary tests are included in the full-suite count. Read-only semantics and explicit text statuses are asserted; decorative icons are hidden from assistive technology.
