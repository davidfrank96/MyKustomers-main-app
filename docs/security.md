...
The [auth integrity audit](AUTH_SESSION_ROUTING_CONFIRMATION.md) records the requested native 24-hour policy; live configuration remains unverified and unchanged pending project access. Workspace entry distinguishes verified zero memberships, pending-owner recovery, unusable memberships, and separate active-admin authority. Admin lookup errors cannot become zero-business creation decisions. The proxy overwrites the internal request-path header and the layout sanitizes it for login return routing. JWT/refresh, AAL2, RLS, provider settings and private-cache policies are unchanged.
